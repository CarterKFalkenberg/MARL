...
class PredatorsPrey(object):
    ...
    def actor_prey_designed(self):
        # Get current position of the prey
        prey_pos = self.preys_positions[0]

        # Finding empty neighbors for prey to move
        [empty_neighbors, action_to_empty_neighbors] = self.empty_neighbor_finder(prey_pos)

        # If no empty neighbors, stay still
        if not empty_neighbors:
            return self.fix_prey()

        # Calculate weighted distance scores from each predator to each possible move
        distance_scores = []
        for neighbor in empty_neighbors:
            score = 0
            for predator_pos in self.predators_positions:
                distance = np.linalg.norm(np.array(predator_pos) - np.array(neighbor), 1)
                # Weight distances: closer predators are more dangerous
                weight = 1 / (distance + 1)  # Adding 1 to avoid division by zero
                score += weight * distance
            distance_scores.append(score)

        # Choose the move that maximizes the weighted distance score
        max_score = max(distance_scores)
        best_moves = [action_to_empty_neighbors[i] for i, score in enumerate(distance_scores) if score == max_score]
