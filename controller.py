...
class controller(object):
    ...
    # given x shape (num_agents*2+2,), and grid_size
    # outputs x' shape (grid_size, grid_size)
    def indicesToGridFlat(self, x, grid_size):
    ...
    # given: x shape (batch_size, num_agents*2 + 2), and grid_size
    # outputs: x' shape (batch size, grid_size, grid_size)
    def indicesToGridBatch(self, x, grid_size):
        x_new = []
        for val in x:
            x_new.append(self.indicesToGridFlat(val, grid_size))
        return x_new
    ...
    def VDNreplay(self, agents):
        ...
        for agent in agents:
            batch = agent.memory.sample(agent.batch_size, seeds)
            x, y = agent.find_targets_uer(batch)
            # y is shape (batch_size, 5)
            y_list.append(y)
        # we want to turn the agent's 4 seperate y_list into 1
        # y_list is shape (4, batch_size, 5)
        # we need y_tot to be (batch_size, 5)
        
        # (4, 5) -> (1, 5) using (np.sum(list, axis=0))
        # (4, 1, 5)
        # [:, i, :] -> "flattens", np.sum(list, axis=0)
        # do that "batch_size" times
        y_list = np.array(y_list)
        #split_vectors = [np.array(y_list)[:, i, :].reshape(1, -1) for i in range(np.array(y_list).shape[1])]
        for i, x_i in enumerate(x): # approaches for i in range(batch_size)
            # *** Design y_tot ***
            # y_tot = sum of q values
            temp = y_list[:,i,:]    # q values for each agent for the given batch
            y_tot = np.sum(temp,axis=0) # sums over q values for each agent for a given batch
            #y_tot = split_vectors[i]
            y_tot_list.append(y_tot)        
            
        y_tot_list = np.array(y_tot_list).reshape(-1, self.action_size)
        # shape (batch_size=len(x), 5)
        #print("new y_tot_list", y_tot_list)
        #print("x", x)
