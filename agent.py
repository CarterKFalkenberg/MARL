...
from keras.layers import Dense, Lambda, Input, Permute, Concatenate, Conv1D, Reshape
...
class Brain(object):
    ...
    def build_model(self):
        ...
        # we are given the (x,y) of all predators and then the (x,y) of all the prey in list format
        # example input for 2 predators, 1 prey: (2, 3, 4, 0, 0, 1)
        #perm = Permute((2, 1), input_shape=(2,))(inputs)
        #c1 = Conv1D(1, 3)(inputs)
        #r1 = Reshape((self.state_size,1))(inputs)
        #c1 = Conv1D(1, 3, padding="same")(r1)
        
        # only problem with this currently is that CNN is not taking the "image", but just the indices
        # potential improvement: convert the indices into a 2d "image" representation, each cell being 0 (empty), 1 (predator), 2(prey)
        
        #r2 = Reshape((self.state_size,))(c1)
        ...
        ###################################
        z = Dense(self.action_size, activation="linear")(x4)
...
class Agent(object):
    ...
    def find_targets_uer(self, batch):
        ...
        for i in errors:
            tot += i
        self.errors.append(tot/len(errors))
