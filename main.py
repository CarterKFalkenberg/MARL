...
class Environment(object):
    ...
    def __init__(self, arguments):
        ...
        self.first_N_rewards = 0
        self.last_N_rewards = 0

        self.first_N_steps = 0
        self.last_N_steps = 0

        self.first_N_reached_goal = 0
        self.last_N_reached_goal = 0
    ...
    def run(self, agents, file1, file2):
        ...
        for episode_num in xrange(self.episodes_number):
            ...
            print("Episode {p}, Score: {s}, Final Step: {t}, Goal: {g}".format(p=episode_num, s=reward_all,
                                                                          ...
                                                                          t=time_step, g=done))
            if episode_num <= self.N:
                self.first_N_rewards += reward_all
                self.first_N_steps += time_step
                if done:
                    self.first_N_reached_goal += 1
                
            if episode_num >= self.episodes_number - 1000:
                self.last_N_rewards += reward_all
                self.last_N_steps += time_step 
                if done:
                    self.last_N_reached_goal += 1
...
if __name__ =="__main__":
    ...
    avg_score_first_n = env.first_N_rewards / env.N 
    avg_score_last_n = env.last_N_rewards / env.N
    avg_steps_first_n = 1.0 * env.first_N_steps / env.N
    avg_steps_last_n = 1.0 * env.last_N_steps / env.N
    percent_goal_first_n = 1.0 * env.first_N_reached_goal / env.N
    percent_goal_last_n = 1.0 * env.last_N_reached_goal / env.N
    
    print("Score (first, last), steps (first, last), % goal (first, last)")
    print(avg_score_first_n, avg_score_last_n, avg_steps_first_n, avg_steps_last_n, percent_goal_first_n, percent_goal_last_n)
